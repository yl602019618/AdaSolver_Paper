...
wall-shear-stress predictions. The available original Airfoil checkpoint
predicts Mach rather than pressure, so its curve is omitted. Both Airfoil
panels use the shared pressure backbone's full-mesh error on the same 110
test cases. Airfoil $C_D/C_L$ uses absolute ratio error before normalization.
Its axis is linear through five and logarithmic above five, with the
transition and four $75\%$ means labeled. Elasticity uses a logarithmic axis.
...
$1.026$, using $25\%$ fewer points than the shared backbone's full mesh.
...
\begin{figure}[!ht]
    ...
    reference $C_L>0.02$ and has no matched Transolver curve. Its $C_D/C_L$
