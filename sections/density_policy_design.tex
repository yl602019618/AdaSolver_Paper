...
\begin{equation}
    ...
    \lambda_{\mathrm{tar}}^{\mathrm{field}}
       &=\mathcal{N}[(0.1+\mathcal{Q}_{0.95}[s_f])^{1/2}],\\
...
\paragraph{Airfoil.}
Pressure gradients identify flow variation relevant to field reconstruction and aerodynamic prediction. A $128\to8\to32\to1$ head predicts pressure, and both metrics use the same density rule. The coarse budget is $10\%$ for the field and $5\%$ for $C_D/C_L$, always including the 121 surface nodes. Denoting the pressure estimate for target $q$ by $\widehat p^{\mathrm{head},q}$ gives
\begin{equation}
    s_q=\mathcal{I}_0G_{16}[\widehat p^{\mathrm{head},q}],
    \qquad
    \lambda_{\mathrm{tar}}^q
       =\mathcal{N}[(0.1+\mathcal{Q}_{0.95}[s_q])^{1/2}],
