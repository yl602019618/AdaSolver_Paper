...
a learned early refinement signal and a cache-aware final forward pass.

Keeping the backbone frozen, we train a small head $H_\phi$ on the first-block
features $h_i^{(1)}$ to predict a task-specific refinement signal,
\begin{equation}
    z_i = H_\phi\!\left(h_i^{(1)}\right),
    \qquad
    s_{\mathrm{CA}}(x_i)=\Psi(z_i),
    \label{eq:learned_refinement_score}
\end{equation}
where $\Psi$ is either a fixed local-variation operator or the identity when
the head directly predicts the refinement score. The resulting score defines
the adaptive sampling density through
\eqref{eq:unified_refinement_measure}, avoiding a full coarse inference.
