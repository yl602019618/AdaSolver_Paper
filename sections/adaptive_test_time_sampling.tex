...
\end{equation}
and use it in Eq.~\eqref{eq:unified_refinement_measure} to sample the final point set.

This policy does not require a preliminary network evaluation; once the prior indicator is available, points are sampled from $\widetilde{\rho}_{\gamma,\mathrm{prior}}$ and inferred with the correction in Eq.~\eqref{eq:unified_refinement_correction}.
...
\end{equation}
where $\Phi$ is a fixed nonnegative combination and the derivatives are evaluated with the available mesh or point-cloud neighborhood. The score is transferred to candidate points, normalized as in Eq.~\eqref{eq:unified_refinement_measure}, and used to sample $\widetilde{\rho}_{\gamma,\mathrm{coarse}}$.
...
\end{equation}
and use this score in Eq.~\eqref{eq:unified_refinement_measure}. Token attention, deslicing, and later blocks still run on $S_1$, because $B$ changes the global token states.
